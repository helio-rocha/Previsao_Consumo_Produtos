...
def display_page(pathname):
    if pathname == '/page-1':
        ...
        html.Div([
        html.Div(dcc.Graph(id='scatter-plot-1'), className="col-12 col-md-6"),  # Coluna 1
        html.Div(dcc.Graph(id='scatter-plot-2'), className="col-12 col-md-6"),  # Coluna 2
    ], className="row"),  # Linha 1

    html.Div([
        html.Div(dcc.Graph(id='scatter-plot-3'), className="col-12 col-md-6"),  # Coluna 3
        html.Div(dcc.Graph(id='scatter-plot-4'), className="col-12 col-md-6"),  # Coluna 4
    ], className="row"),  # Linha 2
...
def update_graph(n):
    ...
    #     graficos.append() = GraficoProduto(go.Figure(data=go.Scatter(x=dff['data'].loc[dff['id_produto'] == id], y=dff['quant'].loc[dff['id_produto'] == id], mode="lines")
    # produtos.loc[df['id_produto'] == 1, 'nome_produto']
        
    grafico1 = GraficoProduto(go.Figure(data=go.Scatter(x=dff['data'].loc[dff['id_produto'] == 1], y=dff['quant'].loc[dff['id_produto'] == 1], mode="lines")), 1, produtos.loc[produtos['id_produto'] == 1, 'nome_produto'].values[0])
    grafico2 = GraficoProduto(go.Figure(data=go.Scatter(x=dff['data'].loc[dff['id_produto'] == 2], y=dff['quant'].loc[dff['id_produto'] == 2], mode="lines")), 2, produtos.loc[produtos['id_produto'] == 2, 'nome_produto'].values[0])
    grafico3 = GraficoProduto(go.Figure(data=go.Scatter(x=dff['data'].loc[dff['id_produto'] == 3], y=dff['quant'].loc[dff['id_produto'] == 3], mode="lines")), 3, produtos.loc[produtos['id_produto'] == 3, 'nome_produto'].values[0])
    grafico4 = GraficoProduto(go.Figure(data=go.Scatter(x=dff['data'].loc[dff['id_produto'] == 4], y=dff['quant'].loc[dff['id_produto'] == 4], mode="lines")), 4, produtos.loc[produtos['id_produto'] == 4, 'nome_produto'].values[0])

    
    grafico1.grafico.update_layout(uirevision='some-constant', showlegend=False, xaxis_autorange=True, yaxis_autorange=True, autosize=True, title=grafico1.name)
    grafico2.grafico.update_layout(uirevision='some-constant', showlegend=False, xaxis_autorange=True, yaxis_autorange=True, autosize=True, title=grafico2.name)
    grafico3.grafico.update_layout(uirevision='some-constant', showlegend=False, xaxis_autorange=True, yaxis_autorange=True, autosize=True, title=grafico3.name)
    grafico4.grafico.update_layout(uirevision='some-constant', showlegend=False, xaxis_autorange=True, yaxis_autorange=True, autosize=True, title=grafico4.name)
            
    ranking = ranquamento(df)

    # Retorna os gráficos individualmente para cada Output
    return grafico1.grafico, grafico2.grafico, grafico3.grafico, [html.P(elemento) for elemento in ranking], grafico4.grafico
